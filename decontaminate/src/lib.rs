...
#[pyfunction]
#[pyo3(signature = (file_path, text_key, id_key, contaminated_ids_path, output_path))]
fn filter_contaminated_rust(
    py: Python,
    file_path: &str,
    text_key: &str,
    id_key: Option<&str>,
    ...
    let result = py.allow_threads(move || {
        ...
        let output_file = File::create(&output_path_owned)
            .map_err(|e| format!("Failed to create output file '{}': {}", output_path_owned, e))?;
        ...
        Ok::<_, String>((total_processed, total_kept))
    });

    match result {
        Ok((processed, kept)) => {
            println!("[filter-contaminated-rust] Processed {} docs, kept {} clean docs, wrote to {}", 
                     processed, kept, output_path);
            Ok((processed, kept))
        },
        Err(e_str) => Err(PyValueError::new_err(format!("Filter failed for {}: {}", file_path, e_str))),
    }
}
...
fn fast_decont(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(filter_contaminated_rust, m)?)?;
    Ok(())
}
