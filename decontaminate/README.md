...
4. Convert the index to native format: `python fast_decont/decontam_hf.py build-index-native  --in-pickle "$PICKLE_INDEX_PATH --out-native "$NATIVE_INDEX_PATH"`
...
python decontam_hf.py filter \
  --input-glob "/leonardo_work/AIFAC_L01_028/shared/datasets/raw/ontocord-MixtureVitae-211BT-parquet/*.parquet" \
...
export PICKLE_INDEX_PATH=index.pkl
export NATIVE_INDEX_PATH=index.native.pickle