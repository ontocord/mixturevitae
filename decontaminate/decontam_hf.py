...
try:
    from fast_decont import build_index_native_rust, scan_file_rust, filter_contaminated_rust
except ImportError:
...
def do_filter(ns: argparse.Namespace) -> None:
    """Filter out contaminated documents from dataset shards."""
    
    # Step 1: Load contaminated IDs from scan results
    print(f"Loading contaminated doc IDs from: {ns.contaminated_dir}", file=sys.stderr)
    contaminated_ids = set()
    
    contam_files = glob.glob(os.path.join(ns.contaminated_dir, "*.decontam.ndjson"))
    if not contam_files:
        print(f"ERROR: No .decontam.ndjson files found in {ns.contaminated_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    for contam_file in contam_files:
        with open(contam_file, 'r') as f:
            for line in f:
                rec = json.loads(line)
                contaminated_ids.add(rec['doc_id'])
    
    print(f"Loaded {len(contaminated_ids):,} contaminated doc IDs", file=sys.stderr)
    
    # Write IDs to temp file for Rust to read
    temp_ids_path = os.path.join(ns.out_dir, "_contaminated_ids.txt")
    ...
    files = sorted(glob.glob(ns.input_glob))
    if not files:
        print(f"ERROR: No files matched glob: {ns.input_glob}", file=sys.stderr)
        sys.exit(1)
    
    print(f"Found {len(files):,} files to filter", file=sys.stderr)
    
    # Step 3: Filter each file
    total_processed = 0
    total_kept = 0
    ...
    for i, file_path in enumerate(files, 1):
        base = os.path.basename(file_path)
        stem = re.sub(r"\.(parquet|jsonl|jsonl\.gz|jsonl\.zst)$", "", base)
        out_path = os.path.join(ns.out_dir, "clean", f"{stem}.clean.jsonl")
        
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        
        try:
            processed, kept = filter_contaminated_rust(
                file_path=file_path,
                text_key=ns.text_key,
                id_key=ns.id_key,
                contaminated_ids_path=temp_ids_path,
            ...
            if i % 10 == 0:
                print(f"[filter] Progress: {i}/{len(files)} | Kept: {total_kept:,}/{total_processed:,} ({100*total_kept/max(total_processed,1):.2f}%)", file=sys.stderr)
        
        except Exception as e:
            print(f"[filter:ERROR] Failed to process {base}: {e}", file=sys.stderr)
    
    print(f"\n[filter] DONE: Processed {total_processed:,} docs, kept {total_kept:,} clean docs ({100*total_kept/max(total_processed,1):.2f}%)", file=sys.stderr)
...
def main():
    ...
    flt = sub.add_parser("filter", help="Filter out contaminated documents from dataset shards.")
    flt.add_argument("--input-glob", required=True, help="Glob for input shards (e.g., './hf_cache/path/to/*/*.parquet').")
    flt.add_argument("--text-key", default="text", help="Field name containing document text (default: text).")
    flt.add_argument("--id-key", default=None, help="Optional field name for doc id.")
    flt.add_argument("--contaminated-dir", required=True, help="Directory with contaminated_docs/*.decontam.ndjson from scan step.")
    flt.add_argument("--out-dir", required=True, help="Directory for cleaned output files.")
    flt.set_defaults(func=do_filter)
