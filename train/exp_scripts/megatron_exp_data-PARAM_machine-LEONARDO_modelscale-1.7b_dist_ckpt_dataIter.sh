...
name_template="open-sci-ref_model-${model}_data-${data}_tokenizer-${tokenizer}_samples-${tokens_num_string}_global_bs-${global_bs}_context-${seq_length}_schedule-${lr_schedule}_lr-${lr}_warmup-${lr_warmup_iters}_machine-${machine_name}"
...
#SBATCH --output=<slurm_output>/${name_template}_%j.out" > ${template_script_path}/exp_${name_template}.sbatch
...
EXP_LABEL="open-sci-ref_model-1.7b_data-${DATA_NAME}_samples-${TOTAL_TOKENS_LABEL}_global_bs-${GLOBAL_BATCH_SIZE}_context-${SEQ_LENGTH}_schedule-${LR_DECAY_STYLE}_lr-${LR}_warmup-${LR_WARMUP_ITERS}"
...
echo "DEBUG: executing sbatch ${template_script_path}/exp_${name_template}.sbatch $micro_bs $gas $lr_schedule $lr $lr_min $lr_warmup_iters $tokens $seq_length $save_interval_iter $data_name $data_switch"
echo "DEBUG: data chunks index to compose: 0 - ${data_chunks}" 

sbatch ${template_script_path}/exp_${name_template}.sbatch $micro_bs $gas $lr_schedule $lr $lr_min $lr_warmup_iters $tokens $seq_length $save_interval_iter $data_name $data_switch